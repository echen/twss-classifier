...
class String  
  def to_ngrams(n)
    self.normalize(n).split.each_slice(n).to_a.map{ |x| x.join(" ") }
  end
...
class NaiveBayes
  ...
  def classify(sentence)
    probs = sentence.to_ngrams(@ngram_size).map{ |word| @probs[word] || [0.5, 0.5] }
  ...
  def get_ngram_counts(sentences)
    ...
    sentences.each do |sentence|
      sentence.to_ngrams(@ngram_size).each{ |ngram| h[ngram] += 1 }      
    end
    return h    
  end
end